...
import { DocumentType, FormatMode, PaymentStatus, finalizeTransaction, createCustomer, getLatestDocumentNo, type Customer, searchCustomers } from '@ledgerx/firebase-shared';
  ...
  const handleFinalize = async () => {
    ...
    try {
      ...
      if (documentType === DocumentType.FINAL_SALE) {
        ...
        if (jobIds.length > 0) {
          try {
            const { deleteJobCard } = await import('@ledgerx/firebase-shared');
